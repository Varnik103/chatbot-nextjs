...
export async function PATCH(req: Request, { params }: { params: { messageId: string } }) {
  ...
  const msg = await db.collection("messages").findOne({ id: params.messageId })
  ...
  await db.collection("messages").updateOne({ id: params.messageId }, { $set: { content, parts: { text: content } } })
